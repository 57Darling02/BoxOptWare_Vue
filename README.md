...
- **Spring Boot 框架**： API实现后端通讯
- **mariadb开源数据库**：用户资料存储
- **JWT（Json Web Token）**：登录凭证加密技术

后端选用 Spring Boot 框架结合mariadb开源数据库，负责数据存储。前端输入账号密码后，通过 API 在后端进行校验，校验通过后返回 JWT 令牌作为身份凭证，保障系统安全。用户设置的货物信息和自定义的集装箱数据存储在后端数据库中，未登录用户的数据则临时存储在网页本地缓存中，方便用户后续操作。
